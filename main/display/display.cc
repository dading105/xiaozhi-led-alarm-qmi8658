...
#include "boards/lichuang-dev/tc5020_led.h" // TC5020Led definition
#include <cstring>
...
void Display::Update() {
    ...
    // Display time on TC5020 LED (RTTI-safe version)
    Led* led = Board::GetInstance().GetLed();
    if (led && Application::GetInstance().GetDeviceState() == kDeviceStateIdle) {
        // Try to cast using a type-checking method
        if (strcmp(led->GetDriverName(), "TC5020Led") == 0) {
            TC5020Led* tc5020_led = static_cast<TC5020Led*>(led);
            
            // Get current time
            time_t now = time(nullptr);
            tm* timeinfo = localtime(&now);
            
            // Format time as HHMM (4-digit format)
            char time_str[5];
            snprintf(time_str, sizeof(time_str), "%02d%02d", 
                     timeinfo->tm_hour, timeinfo->tm_min);
            
            // Set each digit on the LED display
            for (int i = 0; i < 4; i++) {
                char digit_char = time_str[i];
                uint8_t digit_value = 0;
                
                // Convert char to 7-segment value
                switch (digit_char) {
                    case '0': digit_value = 0x3F; break;
                    case '1': digit_value = 0x06; break;
                    case '2': digit_value = 0x5B; break;
                    case '3': digit_value = 0x4F; break;
                    case '4': digit_value = 0x66; break;
                    case '5': digit_value = 0x6D; break;
                    case '6': digit_value = 0x7D; break;
                    case '7': digit_value = 0x07; break;
                    case '8': digit_value = 0x7F; break;
                    case '9': digit_value = 0x6F; break;
                    default:  digit_value = 0x00;  // Blank for invalid
                }
                tc5020_led->SetDigit(i, digit_value);
            }
            tc5020_led->Refresh();
        }
    }
